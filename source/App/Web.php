...
class Web extends Controller
{
    ...
    {
        $head = $this->seo->render(
            "Sobre ".CONF_SITE_NAME . " - " . CONF_SITE_DESC,
            CONF_SITE_DESC,
            url("/sobre"),
            theme("/assets/images/share.jpg")
        );
    ...
    {
        $head = $this->seo->render(
            " Lançamentos da ".CONF_SITE_NAME . " - " . CONF_SITE_DESC,
            CONF_SITE_DESC,
            url("/lancamentos"),
            theme("/assets/images/share.jpg")
        );

        echo $this->view->render("releases", [
            "head" => $head,

        ]);
    }

    public function predio1(): void
{
    $head = $this->seo->render(
        "Lançamento Emily XVIII".CONF_SITE_NAME . " - " . CONF_SITE_DESC,
        CONF_SITE_DESC,
        url("/predio1"),
        theme("/assets/images/share.jpg")
    );

    echo $this->view->render("predio1", [
        "head" => $head,
    ...
    public function predio2(): void
    {
        $head = $this->seo->render(
            "Lançamento Emily XX" .CONF_SITE_NAME . " - " . CONF_SITE_DESC,
            CONF_SITE_DESC,
            url("/predio2"),
            theme("/assets/images/share.jpg")
        );

        echo $this->view->render("predio2", [
            "head" => $head,
     ...
     * @param null|array $data
     */
    public function exemplo(?array $data): void
    {
        if (!empty($data['csrf'])) {
            if (!csrf_verify($data)) {
                $json['message'] = $this->message->error("Erro ao enviar, favor use o formulário")->render();
            ...
            if (in_array("", $data)) {
                $json['message'] = $this->message->info("Informe seus dados para abaixar o livro.")->render();
                echo json_encode($json);
                return;
            }
            ...
            if ($auth->reset($data["name"], $data["phone"], $data["email"])) {
                $json["redirect"] = url("/pdf");
            } else {
                $json["message"] = $auth->message()->before("Ooops! ")->render();
            }
        ...
        echo $this->view->render("exemplo", [
            "head" => $head,
        ]);
    }

    public function Exemplo2(?array $data): void
    {
        ...
        echo $this->view->render("exemplo2", [
            "head" => $head,
        ]);
    }
    ...
    {
        $head = $this->seo->render(
            "Seu Contato Foi Confirmado - " . CONF_SITE_NAME,
            CONF_SITE_DESC,
            url("/mensagem"),
            theme("/assets/images/share.jpg")
        );
        ...
        echo $this->view->render("message", [
            ...
            "data" => (object)[
                ...
                "desc1" => "Em breve entraremos em contato com você a respeito da sua mensage",
                "desc2" => "Ou se preferir utilize nossos canais de atendimento:",
                "desc3" => "E-mail: comercial@codificar.com.br / Fixo: (31) 2526-1500 / Whatsapp: (31) 9 9810-7504",
                "image" => theme("/assets/images/optin-confirm.jpg")
            ]
        ]);
    }
